...
char* indexes(const char* path)
{
    ...
    char * path_copy = malloc(strlen(path)+strlen("index.html")+1);
    strcpy(path_copy,path);

    if (strcasecmp(path_copy, "index.php")==0)
    {
        strcat(path_copy,"index.php");
        return path_copy;
    }
    if (strcasecmp(path_copy, "index.html")==0)
    {
        strcat(path_copy,"index.php");
        return path_copy;
    }
...
const char* lookup(const char* path)
{
    //compare path to given scription, if matching, return output
    if (path != NULL)
    {
        if (strcasecmp(path,"CSS") ==0)  {return "text/css";}
        if (strcasecmp(path,"HTML")==0)  {return "text/html";}
        if (strcasecmp(path,"GIF") ==0)  {return "text/gif";}
        if (strcasecmp(path,"ICO") ==0)  {return "text/ico";}
        if (strcasecmp(path,"jpg") ==0)  {return "text/jpeg";}
        if (strcasecmp(path,"JS")  ==0)  {return "text/javascript";}
        if (strcasecmp(path,"PHP") ==0)  {return "text/x-php";}
        if (strcasecmp(path,"PNG") ==0)  {return "text/png";}
    }
    //no path, return NULL
    return NULL;
}
...
bool parse(const char* line, char* abs_path, char* query)
{
    // TODO
        ...
        char* abs_pathcopy;
        char* querycopy;
        abs_pathcopy = strtok(request_target, "?");
